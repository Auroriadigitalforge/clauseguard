...
export const ClauseCard: React.FC<ClauseCardProps> = ({ clause, index, onAskLawyer }) => {
  ...
  return (
    ...
    >
      {/* Top Header */}
      <div className={`p-4 sm:p-5 border-b border-slate-100 ${config.headerBg} flex flex-col sm:flex-row sm:items-center justify-between gap-2.5`}>
        <div className="flex items-start sm:items-center gap-2.5">
          <span className="text-xs font-mono font-bold text-slate-400 bg-white px-2 py-0.5 rounded border border-slate-200 shrink-0">
            #{index + 1}
          </span>
        ...
        <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
          {onAskLawyer && clause.riskLevel !== 'SAFE' && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-white hover:bg-amber-50 text-amber-900 border border-amber-300 shadow-2xs transition-colors cursor-pointer"
          ...
          <div className="pt-1">
            <div className="flex items-center justify-between gap-2 mb-1.5">
              ...
              <button
                ...
                className="text-xs font-medium text-indigo-600 hover:text-indigo-800 flex items-center gap-1 py-0.5 px-2 rounded hover:bg-indigo-50 transition-colors"
        ...
        {clause.originalSnippet && (
          <div className="pt-1 border-t border-slate-100">
            <button
              ...
              className="flex items-center justify-between w-full text-xs font-medium text-slate-500 hover:text-slate-800 py-1"
            >
            ...
            {showOriginal && (
              <div className="mt-2 p-3 bg-slate-100/80 rounded-lg text-xs font-mono text-slate-600 border border-slate-200/70 leading-relaxed whitespace-pre-wrap">
                {clause.originalSnippet}
              </div>
            )}
          </div>
